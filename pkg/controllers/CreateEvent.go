...
func (c controller) CreateEvent(w http.ResponseWriter, r *http.Request) {
    ...
    if !ok || len(projectParam[0]) < 1 {
        c.ErrorResponse(w, "Url param project_id is required.")
		return
    }
	...
	if eventNum != len(eventKeys) {
        c.ErrorResponse(w, "The number of event_value does not match the number of event_key.")
		return
    }
	...
	if result := c.DB.Create(&events); result.Error != nil {
		c.ErrorResponse(w, "Failed to create events in DB.")
		return
	}
